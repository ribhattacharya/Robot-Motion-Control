...
function [V, u, theta_dot, T0e, Xerr] = FeedbackControl(X, Xd, Xd_next, Kp, Ki, t, thetalist)
...
%% Finding twist
    ...
    % Calculating desired twist from current desired end-effector
    % configuration and next desired end-effector configuration
    Vd_skew = (MatrixLog6((TransInv(Xd))*Xd_next))/t;
    Vd = se3ToVec(Vd_skew);
    
    % Calculating twist for current iteration with feed forward and
